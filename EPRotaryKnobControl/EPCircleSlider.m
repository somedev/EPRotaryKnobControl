...
//handle diameter
static const CGFloat kHandleDiameterRatio =0.1f;
...
@property(nonatomic) BOOL agleUpdated;
@end
...
- (void)setupView:(CGRect)frame
{
    ...
    self.handleLayer.frame=self.circleLayer.frame;
...
#pragma mark UIControl Tracking
...
- (double)calculateAngleFromTouchPoint:(CGPoint)point{
    ...
    CGFloat len = (CGFloat) sqrt(polarXY.x*polarXY.x + polarXY.y*polarXY.y);
    polarXY.x /= len;
    polarXY.y /= len;
...
- (void)layoutSublayersOfLayer:(CALayer *)layer{
    ...
    CGFloat circleDiameter= fminf(self.frame.size.width, self.frame.size.height);
    double handleRadius= kHandlePositionRatio *circleDiameter/2;
    CGPoint position= [self calculatePointPositionWithAngle:self.currentAngleRadians radius:handleRadius];
    UIBezierPath *path= [[UIBezierPath alloc] init];
    [path addArcWithCenter:[self.handleLayer convertPoint:position fromLayer:self.layer]
                    radius:circleDiameter*kHandleDiameterRatio/2
                startAngle:0
                  endAngle:(CGFloat) (2* M_PI)
                 clockwise:YES];

    self.handleLayer.strokeColor=self.handleAndDotsColor.CGColor;
    self.handleLayer.fillColor= self.handleAndDotsColor.CGColor;
    ...
    if(self.dotsCount>0){
        ...
        self.dotsLayer.fillColor=self.handleAndDotsColor.CGColor;
